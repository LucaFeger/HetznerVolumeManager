...
SERVER_OBJECT=$(jq '.servers[]|select(.public_net.ipv4.ip=="'"$IP"$'")' <<< "$SERVER_RESPONSE")
SERVER_ID=$(jq ".id" <<< "$SERVER_OBJECT")
...
ERROR_CHECK=$(jq '.error.message' <<< "$HTTP_BODY") 

if [ "$ERROR_CHECK" = "null" ]; then
	echo "Configuring volume..."

	LINUX_TEMP=$(jq '.volume.linux_device' <<< "$HTTP_BODY")
